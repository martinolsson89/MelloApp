...
// Define a type for your user info DTO
interface UserDto {
    id: string;
    email: string;
    firstName: string;
...
function BetOverview() {
    ...
    // Fetch the logged-in user info to check if they have placed a bet
    ...
    async function fetchBetOverview() {
        try {
    ...
    useEffect(() => {
        if (!isLoggedIn) return; // If user is not logged in, skip fetching

        setIsLoading(true);

        // Fetch all data in parallel
        Promise.all([fetchUserData(), fetchBetOverview(), fetchFinalPrediction(), fetchResultsData()])
            .then(() => {
                setIsLoading(false);
            })
            .catch((error) => {
    ...
    // If user is not logged in, you could simply return a prompt or redirect
    if (!isLoggedIn) {
        return (
            <Box sx={{ mt: 4, textAlign: 'center' }}>
                <Typography variant="h6" color="error">
                    Du är inte inloggad. Logga in för att fortsätta.
                </Typography>
            </Box>
        );
    }
                ...
                !hasBet ? (
                    ...
                    >
                        <Typography variant="h6">
                            Vänligen gå till röstningsidan och skicka in ditt tips innan du kan se de andra användarnas tips.
                        </Typography>
                    </Box>
                ) : (
                    // ------------------------
                    // If user has placed a bet, display the bet overview
                    <Box
                        sx={{
                            mt: 4,
                            textAlign: 'left',
